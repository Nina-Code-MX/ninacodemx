                            ...
                            @foreach ($data as $d) 
                                ...
                                <tr class="border-b border-neutral-200 duration-300 ease-in-out flex flex-col hover:bg-neutral-100 items-start md:table-row transition ">
                                    ...
                                    @foreach ($dd as $key => $data_info) 
                                        ...
                                        <td class="whitespace-nowrap lg:px-6 lg:py-4 @if (!in_array($key, ['select_value'])) hidden @endif lg:table-cell lg:w-auto p-0 w-full" data-key="{{ $key }}">
                                            @if ($key === 'image') 
                                                    ...
                                                    @else
                                                        <div class="flex items-center justify-between w-full">
                                                            <span class="grow lg:p-0 px-4 py-2">
                                                                <span class="break-words whitespace-break-spaces">{{ \Illuminate\Support\Str::words($data_info ?? '-', 20, '... ') }}</span>
                                                                <span class="block italic lg:hidden text-xs">{{ $d->created_at ?? '-' }}</span>
                                                            </span>
